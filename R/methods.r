...
#              m[j,i] <- paste("\\includegraphics[height=1.4em]{", fn, "}",sep="")
    ...
    if(outputType=="tex"){
      outputMat <- m
      print(xT <- xtable(m, align=c('p{7cm} ','c ','c ','c ','c ','c ','c ','c ','c ','c ','c ','c ','c ','c ','c')), 
    ...
    if(!is.null(filename)){
      ...
      if(outputType%in%c("html","htmlsvg")){
        ...
        cat('}\n')
        cat('\\begin{document}\n')
        print(xT <- xtable(m, align=c('p{7cm} ','c ','c ','c ','c ','c ','c ','c ','c ','c ','c ','c ','c ','c ','c')), sanitize.text.function = function(x){x})
        cat('\\end{document}')
      }
      sink()
    }
